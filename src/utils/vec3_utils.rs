...
mod tests {
    ...
    fn test_cross_itself() {
        let test_vector = Vec3::new_with_values(1.0, 1.0, 1.0);
        let test_vector2 = Vec3::new_with_values(1.0, 1.0, 1.0);
        let zero_vector = Vec3::new_with_values(0.0, 0.0, 0.0);
        assert_eq!(cross(&test_vector, &test_vector2), zero_vector);
    }

    #[test]
    fn test_cross_orthogonal() {
        let x_vector = Vec3::new_with_values(1f64, 0.0, 0.0);
        let y_vector2 = Vec3::new_with_values(0.0, 1f64, 0.0);
        let z_vector = Vec3::new_with_values(0.0, 0.0, 1f64);
    ...
    fn test_unit_vector() {
        let test_vector = Vec3::new_with_values(1.0, 1.0, 1.0);
        debug_assert_eq!(unit_vector(test_vector), test_vector / 3.0_f64.sqrt());
    }
}
